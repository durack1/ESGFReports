...
os.chdir(homePath)
csvFiles = glob.glob(os.path.join(
    timeFormatDir, '*_datasets_*_footprint_*.csv'))
csvFiles.sort()
# Create dictionary
files = {}
...
for count, filePath in enumerate(files):
    ...
    # Compare time axes
    inds = np.intersect1d(
        dateList, tmpTime, assume_unique=True, return_indices=True)
...
for val in x:
    ...
    for count, val in enumerate(list(files.keys())):
        ...
        actLabels.append(
            ' '.join([val, str(int(arr2[step, count+offset]/1e12))]))
    # print(actLabels)
    ...
    leg2 = plt.legend(loc='upper left', ncol=2, prop={
                      "family": "Microsoft Sans Serif", "size": 8})
    plt.title(''.join(
        ['Federated CMIP6 cumulative dataset footprint (Updated: ',
         timeNow.strftime('%Y-%m-%d'), ')']), fontname="Microsoft Sans Serif")
    plt.xlabel('Date', fontname="Microsoft Sans Serif")
    plt.ylabel('Dataset size (PB, 1e15)', fontname="Microsoft Sans Serif")
    ...
    plt.xticks(fontname="Microsoft Sans Serif")
    plt.subplots_adjust(left=.06, right=.96, bottom=.08,
                        top=0.95, wspace=0, hspace=0)
    anoStr = ''.join(
        ['CMIP6 total \'latest\' datasets (TB, 1e12): ',
    ...
    plt.annotate(anoStr, xy=(dateTweak, datasetNumTweak),
                 xytext=(dateTweak, datasetNumTweak), fontname="Microsoft Sans Serif")
    print('dateList:', dateList[step])
    print("timeCount:", str(step), step)
    plt.savefig(os.path.join("demo", '_'.join(
